...
Mesh::Mesh(char *name, char *path) : Resource< Mesh >(name, path)
{
	...
	D3DXLoadMeshHierarchyFromX(
		...
		D3DXMESH_MANAGED,
		g_engine->GetDevice(),
		&ah, NULL, (D3DXFRAME**)&m_firstFrame,
...
#pragma region Tạo danh sách static Mesh
	...
	D3DXLoadMeshFromX(
		...
		&materialBuffer,
		NULL,
		&m_staticMesh->NumMaterials,
		&m_staticMesh->originalMesh);

	// Optimise the mesh for better rendering performance.
	m_staticMesh->originalMesh->OptimizeInplace(
...
{
	m_frames->Add(frame);// Có được list frame
	...
	if (frame->pMeshContainer != NULL)
	{
		...
		else
		{
			...
			for (unsigned long m = 0; m < meshContainer->NumMaterials; m++)
			{
				...
				//When you call DrawSubset, what it's actually doing (this is of course simplified), 
				//is a SetStreamSource, SetIndices, and a DrawIndexedPrimitive
