...
void SceneManager::LoadScene(char *name, char *path)
{
	...
	sun.Direction = D3DXVECTOR3(
		script->GetVectorData("sun_direction")->x, 
	...
	D3DXMatrixPerspectiveFovLH(&projection,
		...
		0.1f / m_scale,
		m_mesh->GetBoundingSphere()->radius * 2.0f);
	...
	//Tao danh sách mesh
#pragma region Tìm các Static mesh có chung Material, lưu vào Rendercache list
	// List m_renderCache chứa các renderCache chung material
...
#pragma endregion
	...
	{
#pragma region Các Occluder được xác định sẵn khi xây dựng Map dùng 3Ds Max, lưu vào list m_occludingObjects
		...
		if (strncmp("oc_", frames->GetCurrent()->Name, 3) == 0)
		{
			...
			m_occludingObjects->Add(new SceneOccluder(
...
#pragma region Tạo list các dynamicObject bao gồm player and spawner
...
#pragma region Danh Sách Valid Faces(có material), có được totalFaces, và mảng validFaces
	...
	for (unsigned long f = 0; f < m_mesh->GetStaticMesh()->originalMesh->GetNumFaces(); f++)
	{
		...
		while (m_renderCaches->Iterate())
		{
			...
			{
				m_totalFaces++;//Total faces valid
				validFaces[f] = true;
				break;
			}
		}
...
#pragma endregion
	...
	m_faces = new SceneFace[m_totalFaces];// SceneFace* m_faces = new SceneFace[m_totalFaces]
	...
	for (unsigned long f = 0; f < m_totalFaces; f++)
	{
		...
		//Với mỗi renderCache ta thêm các face có cùng material vào
		m_renderCaches->Iterate(true);
		while (m_renderCaches->Iterate())
		{
			if (m_renderCaches->GetCurrent()->GetMaterial() == m_mesh->GetStaticMesh()->materials[attributes[f]])
	...
	RecursiveSceneBuild(m_firstLeaf,
		m_mesh->GetBoundingSphere()->centre,
		m_mesh->GetBoundingBox()->halfSize);
...
void SceneManager::Update(float elapsed, D3DXMATRIX *view)
{
	...
	while (m_dynamicObjects->Iterate())
	{
		...
		if (m_dynamicObjects->GetCurrent()->GetEllipsoidRadius().x +
			m_dynamicObjects->GetCurrent()->GetEllipsoidRadius().y,
...
void SceneManager::Render(float elapsed, D3DXVECTOR3 viewer)
{
	...
	//Kiem tra Scene Leaf voi view Frustum
	RecursiveSceneFrustumCheck(m_firstLeaf, viewer);
	//Co duoc m_visibleOccluder va m_visibleLeaf
	...
	while (m_visibleOccluders->Iterate())
	{
		...
		// Build vung` che khuat cua moi doi tuong Occluder
		BuildOcclusionVolume(m_visibleOccluders->GetCurrent(), viewer);

#pragma region Loại những occluder bị các occluder gần hơn che khuất
		// Iterate through the rest of the visible occluder's.
// Loại những occluder bị các occluder gần hơn che khuất
		...
		while (occludee != NULL)
		{
			...
			if (IsSphereOverlappingVolume(m_visibleOccluders->GetCurrent()->planes,
				occludee->translation, occludee->GetBoundingSphere()->radius) == true)
				if (IsBoxEnclosedByVolume(m_visibleOccluders->GetCurrent()->planes,
					occludee->GetBoundingBox()->min, occludee->GetBoundingBox()->max) == true)
...
#pragma endregion
	...
	// Kiểm tra các Leaf với Các visible Occluder
	//- Nếu chưa hidden thì render Face đó lên
...
#pragma region Render các Spawner(Gun...) thuộc list dynamic 
...
bool SceneManager::RayIntersectScene(RayIntersectionResult *result, D3DXVECTOR3 rayPosition, D3DXVECTOR3 rayDirection, bool checkScene, SceneObject *thisObject, bool checkObjects)
{
	...
	if (checkScene == true)
	{
		...
		for (unsigned long f = 0; f < m_totalFaces; f++)
		{
			...
			if (D3DXIntersectTri(
				(D3DXVECTOR3*)&m_vertices[m_faces[f].vertex0],
...
void SceneManager::BuildOcclusionVolume(SceneOccluder *occluder, D3DXVECTOR3 viewer)
{
	...
	while (edges->Iterate() != NULL)
	{
		...
		// Calculate the position of the third vertex for creating the plane.
...
//-----------------------------------------------------------------------------
void SceneManager::RecursiveSceneBuild(SceneLeaf *leaf, D3DXVECTOR3 translation, float halfSize)//leaf, spherecenter, box halfsize
{
	...
	unsigned long totalFaces = 0;//of one leaf
	for (unsigned long f = 0; f < m_totalFaces; f++)
		if (IsFaceInBox(
			&m_vertices[m_faces[f].vertex0],
			&m_vertices[m_faces[f].vertex1],
			&m_vertices[m_faces[f].vertex2],
	...
	leaf->faces = new unsigned long[totalFaces];//faces in leaf
...
#pragma region occluder for Leaf
	//Duyệt tất cả các occluder, em nào bi chua trong leaf thi add vao list cac occluder cua leaf do'
	// Store pointers to any occluding objects in the leaf.
	m_occludingObjects->Iterate(true);// occluder in Scene
...
#pragma region Tạo List các Visible Occluder
	//Mỗi leaf chứa 1 danh sách các occluder, duyệt hết danh sách đó nếu occluder đó k nằm trong view Frustum thì bỏ qua
	//- Tính khoảng cách từ các occluder tới eye
	//- Duyệt danh sách các Occluder nhìn thấy, thằng nào gần view hơn thì add trước
	...
	while (leaf->occluders->Iterate())
	{
		...
		while (m_visibleOccluders->Iterate())
		{
			...
			{
				m_visibleOccluders->InsertBefore(
					leaf->occluders->GetCurrent(),
...
void SceneManager::RecursiveSceneOcclusionCheck(SceneLeaf *leaf)
{
	...
	for (unsigned long f = 0; f < leaf->totalFaces; f++)
	{
		...
		m_faces[leaf->faces[f]].renderCache->RenderFace(
			m_faces[leaf->faces[f]].vertex0, m_faces[leaf->faces[f]].vertex1, m_faces[leaf->faces[f]].vertex2);
	}
}