...
Font::Font( char *name, short size, unsigned long bold, bool italic )
{
	...
	g_engine->GetDevice()->CreateVertexBuffer( 1020 * sizeof( TransLight_Vertex ), D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0, D3DPOOL_DEFAULT, &m_vb, NULL );
...
void Font::Render( char *text, float x, float y, D3DCOLOR colour )
{
	...
	TransLight_Vertex* vertices = NULL;
	...
	while( *text )
	{
		...
		if( c != _T( ' ' ) )
		{
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x - 0.5f, y + h - 0.5f, 0.0f, 1.0f ), colour, tx1, ty2 );
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x - 0.5f, y - 0.5f, 0.0f, 1.0f ), colour, tx1, ty1 );
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x + w - 0.5f, y + h - 0.5f, 0.0f, 1.0f ), colour, tx2, ty2 );
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x + w - 0.5f, y - 0.5f, 0.0f, 1.0f ), colour, tx2, ty1 );
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x + w - 0.5f, y + h - 0.5f, 0.0f, 1.0f ), colour, tx2, ty2 );
			*vertices++ = TransLight_Vertex( D3DXVECTOR4( x - 0.5f, y - 0.5f, 0.0f, 1.0f ), colour, tx1, ty1 );
			totalTriangles += 2;
