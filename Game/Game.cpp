...
void Game::Load()
{
	...
	m_crosshair = g_engine->GetMaterialManager()->Add("Crosshair.dds.txt", "./Game/Assets/");
	...
	m_music = new Sound("./Game/Assets/Sounds/music_loop.wav");
	m_music->Play(true);
}
...
void Game::HandleNetworkMessage(ReceivedMessage *msg)
{
	...
	case MSGID_CREATE_PLAYER:
	{
		...
		PlayerInfo* player = g_engine->GetNetwork()->GetPlayer(msg->dpnid);
		...
		if (object->GetID() == g_engine->GetNetwork()->GetHostID())
		{
			...
			g_engine->GetSceneManager()->LoadScene(((PlayerData*)g_engine->GetNetwork()->GetPlayer(msg->dpnid)->data)->map, "./Game/Assets/Scenes/");
