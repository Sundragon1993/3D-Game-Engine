...
//-----------------------------------------------------------------------------
PlayerObject::PlayerObject( PlayerInfo *player, Script *script, unsigned long type ) : Animated_Extended_Object( script->GetStringData( "mesh_name" ), script->GetStringData( "mesh_path" ), type )
{
	...
	Script *weaponScript = new Script( "Gun1.txt", "./Game/Assets/Objects/Gun1/" );
...
void PlayerObject::Update( float elapsed, bool addVelocity )
{
	...
	Animated_Extended_Object::Update( elapsed, addVelocity );
...
void PlayerObject::Render( D3DXMATRIX *world )
{
	...
	if( m_dpnid != g_engine->GetNetwork()->GetLocalID() )
		Animated_Extended_Object::Render( world );
